...
use crate::prelude::{Id, Cloned, CurrentBlockers, CurrentEvents, DummyWaker, EventId, ExecutionGraph, KernelSystem, Memory, NextBlockers, NextEvents, ProcessorSystemRegistry, ResourceId, Shared, StateMachine, StoredSystem, System, SystemCell, SystemEvent, SystemId, SystemMetadata, SystemRegistry, SystemResult, SystemStatus, Unique};

pub mod scheduler;
pub mod tasks;
pub mod processor_system_registry;
pub mod system_event_mapper;
...
impl Processor {
    ...
    pub fn insert_system2(state_machine: &StateMachine, system_registry: &mut SystemRegistry, system_id: SystemId, system_metadata: SystemMetadata, stored_system: StoredSystem) -> Option<Option<SystemMetadata>> {
    ...
    // `Id` == `Source` 👍😁
    pub fn get_systems<'a>(memory: &Memory, system_registry: &'a SystemRegistry) -> HashMap<&'a SystemId, &'a SystemMetadata> {
    ...
    fn divide_independent<'a>(&self, mut systems: HashMap<&'a SystemId, &'a SystemMetadata>) -> impl Iterator<Item = HashMap<SystemId, &'a SystemMetadata>> {
    ...
    pub async fn execute(&self, execution_graphs: Arc<Vec<RwLock<ExecutionGraph<SystemId>>>>, memory: &Arc<Memory>, trace: Arc<Trace>) -> Vec<(SystemId, SystemResult)> {
                        ...
                        'graphs_walk: while {
                            ...
                            if leaf_count > 0 {
                                ...
                                if let Some(system_id) = nth_leaf {
                                    let (resource_id, program_id, key) = system_map.get(&system_id).unwrap();
                                        ...
                                        Ok(mut status) => {
                                            match *status {
                                                SystemStatus::Ready => {
                                                    ...
                                                    if let Some(Ok(())) = inner.reserve_accesses(&memory, program_id.as_ref(), system_id.clone(), key.as_ref()) {} 
                                                    else {
                                                        chain += 1;
                                                    ...
                                                    match inner {
                                                        System::Sync(sync_system) => {
                                                            if trace.enabled_system() {
                                                                println!("----> Thread: {i}. System Running: {:?}", system_id);
                                                            }
                                                            ...
                                                            if let Some(result) = result {
                                                                let _ = results.send((system_id.clone(), result));
                                                            }
...
impl KernelSystem for Processor {
    fn init(&mut self, memory: &Memory) -> ResourceId {
        ...
        ResourceId::from_labelled_heap("KernelProcessor")
    }
