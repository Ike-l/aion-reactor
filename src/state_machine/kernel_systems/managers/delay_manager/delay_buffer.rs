...
use crate::prelude::{CurrentEvents, Delay, DelayRegistry, EventId};
...
impl DelayBuffer {
    ...
    pub fn tick(&mut self, delay_registry: &DelayRegistry, current_events: &mut CurrentEvents) {
        ...
        for delay in self.0.drain(..).collect::<Vec<_>>() {
            match delay.delayed_by.as_ref() {
                ...
                _ => { current_events.insert(delay.then_inserts); },
            };
        }
    }
}
