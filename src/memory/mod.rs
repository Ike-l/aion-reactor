...
use crate::{id::Id, injection::injection_trait::{Injection, MemoryTarget}, memory::{access_checked_heap::heap::{raw_heap_object::RawHeapObject, HeapId, HeapObject }, errors::ResolveError, memory_domain::MemoryDomain, program_memory_map::ProgramMemoryMap, resource_id::Resource}, system::system_metadata::Source};
...
impl Memory {
    ...
    pub fn insert_program(&self, program_id: Id, memory_domain: Arc<MemoryDomain>) -> bool {
        self.program_memory_map.insert(program_id, memory_domain)
    }

    pub fn end_drop_delay(&self, key: u64, program_id: Option<&Id>) -> Option<()> {
        Some(if let Some(program_id) = program_id {
            self.program_memory_map.get(program_id)?.end_drop_delay(&key);
        } else {
    ...
    pub fn ok_resources<T: Injection>(&self, program_id: Option<&Id>, source: Option<&Source>, resource_id: Option<ResourceId>) -> Option<bool> {
        ...
        Some(match T::select_memory_target() {
            ...
            MemoryTarget::Program => access_map.ok_resources(self.program_memory_map.get(program_id.as_ref()?)?) 
        })
    }
    ...
    pub fn ok_accesses<T: Injection>(&self, program_id: Option<&Id>, source: Option<&Source>, resource_id: Option<ResourceId>) -> Option<bool> {
        ...
        Some(match T::select_memory_target() {
            ...
            MemoryTarget::Program => access_map.ok_accesses(self.program_memory_map.get(program_id.as_ref()?)?)
        })
    }
    ...
    pub fn reserve_accesses<T: Injection>(&self, program_id: Option<&Id>, resource_id: Option<ResourceId>, source: Source) -> Option<bool> {
        ...
        Some(match T::select_memory_target() {
            ...
            MemoryTarget::Program => self.program_memory_map.get(program_id.as_ref()?)?.reserve_accesses(source, access_map) 
        })
    }
    ...
    pub fn resolve<T: Injection>(&self, program_id: Option<&Id>, resource_id: Option<&ResourceId>, source: Option<&Source>) -> Option<Result<T::Item<'_>, ResolveError>> {
        let map = match T::select_memory_target() {
            ...
            MemoryTarget::Program => self.program_memory_map.get(program_id.as_ref()?)?
        };
    ...
    pub fn insert<T: 'static>(&self, program_id: Option<&Id>, resource_id: Option<ResourceId>, resource: T) -> Option<Option<Resource>> {
        ...
        Some(if let Some(id) = program_id {
            self.program_memory_map.get(id)?.insert(resource_id.unwrap_or(ResourceId::from(HeapId::from(TypeId::of::<T>()))), Resource::Heap(HeapObject(RawHeapObject::new(resource))))
        } else {
